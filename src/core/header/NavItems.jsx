...
const NavItems = ({ ToggleMenu, MenuOpen, isActive, }) => {
  ...
  return (
    <>
      <nav className={`${MenuOpen ? 'translate-x-0' : '-translate-y-full'} duration-300 mx-auto  poppins-medium   rounded-b-lg text-[#881B1B] bg-white min-h-50 max-h-screen max-w-full w-full top-0 right-0 absolute z-50`}>
        ...
        <div className="p-5">
          ...
          <ul className="flex flex-col items-start mt-0 *:my-2 *:px-2  overflow-y-auto max-h-[calc(100vh-150px)] ">
            <Link to="/" className={`py-1  px-2 ${isActive('/') ? 'bg-[#881B1B] text-white  rounded-lg min-w-[75px] py-1  px-2' : ' '}`}>
              <li
                onClick={() => {
                  ToggleMenu(false);
                }}
            ...
            <li ref={dropdown1Ref} >
              <button
                ...
                className={`flex items-center gap-1 cursor-pointer py-1 px-2 ${about.some(link =>
                  link.children
                    ? link.children.some(sub => isActive(`${sub.path}#${sub.id}`, true))
                    : isActive(link.path)
                )
                  ? 'text-white bg-[#881B1B]   py-1 px-2 rounded-lg'
                  : ''
                  }`}
              >
                About Us
                          ...
                          {link.children ? (
                            <>
                              <button
                                ...
                                className={`w-full flex justify-between items-center px-4 py-1.5 rounded-md cursor-pointer ${isParentActive ? 'text-white bg-[#881B1B]' : ''
                                  }`}
                              >
                                {link.name}
                                <FaChevronDown
            ...
            <Link to="/contact-us" className={` ${isActive('/contact-us') ? 'text-white bg-[#881B1B]  rounded-lg min-w-[75px] py-1  px-2' : ''}`}>
              <li
                onClick={() => {
                  ToggleMenu(false);
